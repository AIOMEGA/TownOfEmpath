...
namespace TownOfHost
{
    ...
    class GameEndChecker
    {
        ...
        public static void SetPredicateToBunnyHunt() => predicate = new BunnyHuntGameEndPredicate();
    ...
    // BunnyHunt
    class BunnyHuntGameEndPredicate : GameEndPredicate
    {
        public override bool CheckForEndGame(out GameOverReason reason)
        {
            reason = GameOverReason.ImpostorByKill;
            ...
            if (Imp == 0 && Crew == 0 && Jackal == 0) //Annihilation
            {
                reason = GameOverReason.ImpostorByKill;
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.None);
            }
            if (Options.SheriffUltraHard.GetBool())
            {
                if(Imp == 0 && Jackal > Crew)
                {
                    reason = GameOverReason.ImpostorByKill; // Jackal Out Number
                    CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Jackal);
                }
                else if (Jackal == 0 && Imp > Crew)
                {
                    reason = GameOverReason.ImpostorByKill; // Imp Out Number
                    CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Impostor);
                }
                else if (Imp == 0 && Jackal == 0 && Crew > 0)
                {
                    reason = GameOverReason.ImpostorByKill; // Sheriff Last Standing
                    CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Sheriff);
                }
            }
            if (Options.SheriffUltraHard.GetBool() == false)
            {
                if (Imp == 0 && Jackal == 0 && Crew > 0)
                {
                    reason = GameOverReason.ImpostorByKill; // Sheriff Last Standing
                    CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Sheriff);
                }
            }
            if (Jackal == 0 && Crew < Imp) // imposter outnumber
            {
                reason = GameOverReason.ImpostorByKill;
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Impostor);
            }
            else if (Imp == 0 && Crew < Jackal) // Jackal Outnumber
            {
                reason = GameOverReason.ImpostorByKill;
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Jackal);
                CustomWinnerHolder.WinnerRoles.Add(CustomRoles.Jackal);
            ...
            else if (Jackal == 0 && Imp == 0 && Crew > 0) // Crew win
            {
                reason = GameOverReason.HumansByVote;
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Crewmate);
            }
            else return false; //Victory condition not met
    ...
    class FFAGameEndPredicate : GameEndPredicate
    {
        public override bool CheckForEndGame(out GameOverReason reason)
        {
            ...
            if (Imp == 0 && Jackal == 0) //Annihilation
            {
                reason = GameOverReason.ImpostorByKill;
                CustomWinnerHolder.ResetAndSetWinner(CustomWinner.None);
            }
            else if (Jackal == 0 && Imp == 1) // imposter outnumber
            {
                reason = GameOverReason.ImpostorByKill;
                foreach (var pc in Main.AllPlayerControls)
                {
            ...
            else if (Imp == 0 && Jackal == 1) // Jackal Outnumber
            {
                reason = GameOverReason.ImpostorByKill;
                foreach (var pc in Main.AllPlayerControls)
                {
